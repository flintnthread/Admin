...
const WebProductsScreen: React.FC = () => {
    ...
    const isMobileWeb = winW <= 640;
    const isTabletWeb = winW > 640 && winW <= 1024;
    const isDesktopWeb = winW > 1024;
    ...
    const handleDelete = useCallback(async (id: string) => {
        ...
        if (!product) return;
        if (!(await sweetCrud.confirmDelete("Product", product.name))) return;
        try {
            await deleteProduct(id);
            setProducts(prev => prev.filter(p => p.id !== id));
            void sweetCrud.deleted("Product");
        } catch (e) {
            void sweetError("Error", getApiErrorMessage(e, "Failed to delete product."));
        }
    }, [products]);
    ...
    // Filter panel content extracted to reuse in both sidebar and modal
        ...
        <ScrollView showsVerticalScrollIndicator={false} style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 24 }}>
            ...
            <View style={wst.filterSection}>
                ...
                {TABS.map(tab => {
                    ...
                    return (
                        <TouchableOpacity key={tab.label} style={[wst.filterTabItem, isActive && { backgroundColor: tab.bg }]} onPress={() => { setSelectedTab(tab.label); setVisibleCount(20); if (isMobileWeb || isTabletWeb) setShowMobileFilter(false); }} activeOpacity={0.75}>
    ...
    return (
        <AdminLayout>
            <View style={wst.root}>
                <ScrollView style={wst.pageScroll} showsVerticalScrollIndicator={false} contentContainerStyle={[
                    wst.pageContent,
                    ...
                    <View style={{ paddingHorizontal: 0 }}>
                        ...
                        <View style={[
                            ...
                            <View style={[
                                ...
                                isMobileWeb && { flexWrap: "wrap" },
                            ]}>
                                <TouchableOpacity
                                    style={[wst.navAddBtn, isMobileWeb && { paddingHorizontal: 12, paddingVertical: 8 }]}
                                    onPress={() => router.push('/Addproduct')}
                                    activeOpacity={0.85}
                                >
                                    <MaterialCommunityIcons name="plus" size={16} color={C.navy} />
                                    <Text style={[wst.navAddBtnTxt, isMobileWeb && { fontSize: 12 }]}>Add Product</Text>
                                </TouchableOpacity>
                                <TouchableOpacity
                                    style={[wst.navBulkBtn, isMobileWeb && { paddingHorizontal: 12, paddingVertical: 8 }]}
                                    onPress={() => router.push('/Bulkupload')}
                                    activeOpacity={0.85}
                                >
                                    <MaterialCommunityIcons name="cloud-upload-outline" size={14} color={C.white} />
                                    <Text style={[wst.navBulkBtnTxt, isMobileWeb && { fontSize: 12 }]}>Bulk Upload</Text>
                                </TouchableOpacity>
                            </View>
                        </View>

                        {/* Stats row — 5 cards desktop, 2×grid mobile/tablet */}
                        {isMobileWeb || isTabletWeb ? (
                            <View style={[wst.statsRow, { flexWrap: "wrap", marginTop: isMobileWeb ? -32 : -42 }]}>
                                {[
                                    ...
                                    <View key={i} style={[
                                        wst.statCard,
                                        isMobileWeb
                                            ? { width: "47%" as any, flex: undefined, minWidth: 120 }
                                            : { width: "18%" as any, flex: undefined, minWidth: 120 },
                                    ]}>
                                        <View style={wst.statCardTop}>
                                            ...
                                            <Text style={[wst.statCardVal, { color: stat.color, fontSize: isMobileWeb ? 20 : 26 }]}>{stat.value}</Text>
                                        </View>
                    ...
                    <View style={[
                        wst.contentArea,
                        (isMobileWeb || isTabletWeb) && { flexDirection: "column" },
                    ]}>

                        {/* LEFT FILTER PANEL — desktop only sidebar, mobile/tablet = modal */}
                        {isDesktopWeb && (
                        ...
                        {(isMobileWeb || isTabletWeb) && (
                            <Modal visible={showMobileFilter} animationType="slide" transparent onRequestClose={() => setShowMobileFilter(false)}>
                                <TouchableOpacity style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.4)" }} activeOpacity={1} onPress={() => setShowMobileFilter(false)} />
                                <View style={wst.mobileFilterSheet}>
                                    <View style={wst.mobileFilterHeader}>
                                        <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                                            <Feather name="sliders" size={15} color={C.navy} />
                                            <Text style={wst.mobileFilterTitle}>Filters & Sort</Text>
                                            {activeFilterCount > 0 && (
                                                <View style={wst.filterCountBadge}>
                                                    <Text style={wst.filterCountBadgeTxt}>{activeFilterCount}</Text>
                                                </View>
                                    ...
                                    {activeFilterCount > 0 && (
                                        <TouchableOpacity onPress={clearFilters} style={{ paddingHorizontal: 16, paddingBottom: 4 }}>
                                            <Text style={wst.filterClearAll}>Clear all filters</Text>
                                        </TouchableOpacity>
                                    )}
                        ...
                        <View style={wst.tableArea}>
                            ...
                            <View style={wst.searchBarWrap}>
                                {(isMobileWeb || isTabletWeb) && (
                                    <TouchableOpacity
                                        style={[wst.mobileFilterToggleBtn, activeFilterCount > 0 && { backgroundColor: C.navy }]}
                                        onPress={() => setShowMobileFilter(true)}
                                    >
                                        <Feather name="sliders" size={14} color={activeFilterCount > 0 ? C.white : C.navy} />
                                        {activeFilterCount > 0 && (
                                            <View style={wst.mobileFilterBadge}>
                                                <Text style={wst.mobileFilterBadgeTxt}>{activeFilterCount}</Text>
                                            </View>
                                        )}
                                    </TouchableOpacity>
                                )}
                            ...
                            <View style={[
                                wst.tableToolbar,
                                isMobileWeb && { flexWrap: "wrap", gap: 6, paddingHorizontal: 10 },
                            ]}>
                                <View style={[wst.tableToolbarLeft, isMobileWeb && { flexWrap: "wrap", gap: 6 }]}>
...
const wst = StyleSheet.create({
    ...
    statsRow: { flexDirection: "row", flexWrap: "wrap", gap: 12, marginBottom: 18, marginTop: -42, marginHorizontal: 6, zIndex: 10 },
    statCard: { flex: 1, backgroundColor: C.white, borderRadius: 14, padding: 14, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 2 },
    ...
    // Mobile filter toggle button inside search bar
    mobileFilterToggleBtn: { width: 34, height: 34, borderRadius: 8, backgroundColor: "#EEF1FF", alignItems: "center", justifyContent: "center", marginRight: 8, position: "relative" },
    mobileFilterBadge: { position: "absolute", top: -4, right: -4, backgroundColor: C.orange, width: 14, height: 14, borderRadius: 7, alignItems: "center", justifyContent: "center" },
    mobileFilterBadgeTxt: { fontSize: 8, color: C.white },
    // Mobile/Tablet filter modal sheet
    mobileFilterSheet: { backgroundColor: C.white, borderTopLeftRadius: 24, borderTopRightRadius: 24, maxHeight: "88%" as any, position: "absolute", bottom: 0, left: 0, right: 0, flexDirection: "column" },
    mobileFilterHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 16, paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: C.border },
    mobileFilterTitle: { fontSize: 16, color: C.navyDeep, fontWeight: "600" as any },
    // Filter panel
